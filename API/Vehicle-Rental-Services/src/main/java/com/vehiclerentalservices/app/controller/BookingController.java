...
public class BookingController {
	...
	public ResponseEntity<BookingDto> createBookVechile(@Valid @RequestBody BookingDto book, @PathVariable int uid, @PathVariable int vid) {
		BookingDto books = this.bookingService.createBooking(book, uid, vid);
	...
	@GetMapping("/booking/status/user/{uid}/{flag}")
	public ResponseEntity<List<BookingDto>> getByStatus(@PathVariable int flag, @PathVariable int uid){
		List<BookingDto> list = this.bookingService.getByFlag(flag, uid);
		return new ResponseEntity<List<BookingDto>>(list, HttpStatus.OK);
	}
	
	@GetMapping("/booking/status/{flag}")
	public ResponseEntity<List<BookingDto>> getByFlags(@PathVariable int flag){
		List<BookingDto> list = this.bookingService.getByFlagUser(flag);
		return new ResponseEntity<List<BookingDto>>(list, HttpStatus.OK);
	}
